...
const SCRIPT = [
  { img: '/cover_test_normal.png', duration: 3000, title: 'Cover Test', text: 'Cover Test – Eye Alignment', subtitle: 'Introduction' },
  { img: '/cover_test_normal.png', duration: 4000, title: '', text: 'Both eyes focus together on the same target.', subtitle: 'Normal Vision' },
  { img: '/cover_test_normal.png', duration: 5000, title: '', text: 'Patient is asked to look at a fixed target.', subtitle: 'Fixation' },
  { img: '/cover_test_tropia.png', duration: 8000, title: '', text: 'One eye is covered. Observe the uncovered eye. Movement indicates tropia.', subtitle: 'Tropia' },
  { img: '/cover_test_tropia.png', duration: 8000, title: '', text: 'Eyes are covered alternately. Watch for refixation movement. This indicates phoria.', subtitle: 'Phoria' },
  { img: '/cover_test_diagram.png', duration: 8000, title: 'Eso | Exo', text: 'Inward or outward movement shows the type of deviation.', subtitle: 'Direction' },
  { img: '/cover_test_diagram.png', duration: 8000, title: '', text: 'Compare normal alignment with deviation.', subtitle: 'Comparison' },
  { img: '/cover_test_normal.png', duration: 6000, title: '', text: 'Cover test is simple and effective for detecting eye misalignment.', subtitle: 'Eye Alignment Test' }
];
...
export default function CoverTestVideo({ onClose, onStartTest }) {
    ...
    <div className="fixed inset-0 z-[200] bg-[#020617] flex flex-col overflow-hidden">
      <div className="shrink-0 flex items-center justify-between px-4 py-3 bg-[#0a0f1d]/95 backdrop-blur-sm border-b border-white/5 z-30">
        ...
        <div className="text-center">
          ...
          <h1 className="text-[13px] font-bold text-white uppercase tracking-wide">Cover Test</h1>
        </div>
      ...
      <div className="flex-1 overflow-y-auto">
          ...
          <AnimatePresence>
            {isIdle && (
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="absolute inset-0 flex flex-col items-center justify-center bg-[#020617] px-6 text-center z-20">
                ...
                <h2 className="text-lg sm:text-2xl text-white font-black tracking-tight uppercase italic mb-2">Cover Test</h2>
                <p className="text-slate-400 text-xs max-w-xs mb-5 leading-relaxed">Clinical module demonstrating eye misalignment, tropia, and phoria through the cover test.</p>
        ...
        {isFinished && (
          <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }} className="px-4 pt-4 pb-6 sm:px-6 text-left">
            ...
            <p className="text-slate-400 text-sm leading-relaxed mb-4">Validate your knowledge on the cover test and strabismus with <strong className="text-white">10 clinical MCQs</strong>.</p>
            ...
            <div className={`mt-4 p-3.5 ${A.note} rounded-xl border`}>
              <h4 className="text-white font-bold text-xs mb-1 flex items-center gap-1.5"><Award size={12} className={A.nIcon} /> Pro Tips</h4>
              <ul className="text-slate-400 text-xs leading-relaxed list-disc list-inside mt-2 space-y-1">
                 <li>Watch closely for refixation movement</li>
                 <li>Distinguish between tropia and phoria</li>
                 <li>Always use a strong fixation target</li>
              </ul>
            </div>
          </motion.div>
        )}
