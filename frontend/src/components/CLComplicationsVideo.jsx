...
import { Play, Pause, X, Eye, Volume2, VolumeX, Square, ChevronLeft, Clock, ClipboardCheck, Award, Maximize2, AlertTriangle } from 'lucide-react';
    ...
    <div className="fixed inset-0 z-[200] flex flex-col md:items-center md:justify-center bg-[#020617] md:bg-black/95 md:backdrop-blur-xl overflow-y-auto md:overflow-hidden font-sans">
      ...
      <div className="md:hidden flex items-center justify-between px-6 py-4 border-b border-white/5 bg-[#0a0f1d]">
        ...
        <div className="flex flex-col items-center">
          <span className="text-[10px] font-black text-red-500 uppercase tracking-[0.2em] mb-0.5">Clinical Module</span>
          <h1 className="text-sm font-bold text-white tracking-widest uppercase">Clinical Hub</h1>
        </div>
        <div className="w-10" />
      </div>

      <div className="relative w-full aspect-video md:max-w-6xl md:rounded-3xl overflow-hidden md:ring-1 md:ring-white/10 md:shadow-2xl shadow-red-500/10 shrink-0">
        <button 
          onClick={onClose}
        ...
        <button 
          ...
          className="absolute top-6 right-6 md:right-20 z-50 p-3 bg-white/10 hover:bg-red-500 hover:text-white rounded-full backdrop-blur-md transition-all text-white/70"
        ...
        {isPlaying && (
          <>
            <button 
              ...
              className="absolute top-6 right-20 md:right-32 z-50 p-3 bg-white/10 hover:bg-red-500 hover:text-white rounded-full backdrop-blur-md transition-all text-white/70"
        ...
        {!isPlaying && currentScene === 0 && (
          <div className="absolute inset-0 z-40 flex flex-col items-center justify-center bg-black/60 bg-[url('/cs_video/scene_procedure.png')] bg-cover bg-center">
            ...
            <div className="relative z-10 flex flex-col items-center text-center p-8">
              <div className="w-20 h-20 bg-red-500/20 rounded-full flex items-center justify-center border border-red-500/30 mb-8 border-t-red-400 animate-spin" style={{ animationDuration: '3s' }}>
                <div className="w-16 h-16 bg-red-500/20 rounded-full flex items-center justify-center animate-spin" style={{ animationDuration: '2s', animationDirection: 'reverse' }}>
                  <AlertTriangle className="text-red-400 animate-pulse" size={32} />
                </div>
              </div>
              <span className="text-[10px] font-black text-red-400 uppercase tracking-[0.4em] mb-4">Adverse Reactions</span>
              <h2 className="text-2xl md:text-5xl text-white font-black mb-4 tracking-tighter uppercase italic">CL Complications Test</h2>
              <p className="text-slate-400 font-medium max-w-lg mb-10 leading-relaxed text-sm md:text-base">Comprehensive clinical analysis of complications related to lens overwear, poor hygiene, and mechanical fitting anomalies.</p>
              <button 
                onClick={handleStart}
                className="group bg-red-500 hover:bg-white hover:text-red-500 text-white px-10 py-5 rounded-2xl font-black shadow-2xl shadow-red-500/20 transition-all transform active:scale-95 flex items-center justify-center gap-4 uppercase tracking-[0.2em] text-xs md:text-sm"
              >
            ...
            >
              <div className="bg-black/60 backdrop-blur-2xl p-6 md:p-10 rounded-[2rem] border border-white/5 shadow-2xl">
                <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-2 md:gap-4 mb-4">
                  <h3 className="text-red-400 font-black uppercase tracking-[0.3em] text-[10px] md:text-xs italic">Step {currentScene + 1}: {currentData.title}</h3>
        ...
        {isPlaying && (
           <div className="absolute top-0 inset-x-0 h-1 bg-white/5 z-40">
              <motion.div 
                className="h-full bg-red-500 shadow-[0_0_20px_rgba(239,68,68,0.8)]"
          ...
          <div className="w-full p-8 md:p-12 text-center md:text-left md:flex md:items-center md:justify-between gap-12">
            <div className="flex-1">
              ...
              <div className="flex flex-col md:flex-row md:items-center gap-4 mb-6">
                <div className="w-12 h-12 bg-red-500/20 rounded-xl flex items-center justify-center text-red-400 shrink-0 mx-auto md:mx-0 shadow-inner">
              ...
              <p className="text-slate-400 text-sm md:text-lg font-medium leading-relaxed max-w-2xl mb-10">
                This assessment focuses on your ability to recognize and manage Contact Lens complications. You have **10 clinical MCQs** to validate your proficiency.
              </p>
            ...
            <div className="hidden lg:flex flex-col gap-6 w-80 shrink-0">
              <div className="glass-card p-6 bg-red-500/5 border-red-500/20 rounded-3xl">
                <h4 className="text-white font-bold mb-2 italic flex items-center gap-2">
                  <Award size={16} className="text-red-400" /> Clinical Warning
                </h4>
                <p className="text-xs text-slate-400 leading-relaxed font-medium">In cases of Microbial Keratitis (MK), a patient may present with a painful red eye and a significant corneal infiltrate. This is a medical emergency; the lens must be removed immediately and the patient referred to a corneal specialist.</p>
              </div>
            </div>
          </div>
        </div>
