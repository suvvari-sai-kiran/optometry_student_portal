...
import { Play, Pause, Volume2, VolumeX, Square, ChevronLeft, Clock, ClipboardCheck, Award, RotateCcw, CheckCircle2, Eye, Glasses } from 'lucide-react';

const SCRIPT = [
  { img: '/cs_video/scene_procedure.png', duration: 4000, title: 'Title', text: 'Frame Selection Test', subtitle: 'Choosing the Right Frame' },
  { img: '/cs_video/scene_clinic.png', duration: 6000, title: 'Aim of the Test', text: 'This test helps select the most suitable spectacle frame based on facial features, comfort, and prescription needs.', subtitle: 'Fit • Comfort • Style' },
  { img: '/cs_video/scene_procedure.png', duration: 10000, title: 'Face Shape Consideration', text: 'Frame selection depends on face shape. Round faces suit rectangular frames, while square faces suit round or oval frames.', subtitle: 'Match Frame to Face' },
  { img: '/cs_video/scene_clinic.png', duration: 10000, title: 'Frame Size', text: 'The frame should match the size of the face and align properly with the eyes and nose.', subtitle: 'Proper Size' },
  { img: '/cs_video/scene_procedure.png', duration: 10000, title: 'Bridge Fit', text: 'The bridge should sit comfortably on the nose without slipping or causing pressure.', subtitle: 'Comfortable Fit' },
  { img: '/cs_video/scene_clinic.png', duration: 10000, title: 'Temple Length', text: 'The temples should extend comfortably over the ears without being too tight or loose.', subtitle: 'Proper Length' },
  { img: '/cs_video/scene_procedure.png', duration: 10000, title: 'Lens Compatibility', text: 'Frame selection also depends on lens type. High prescriptions require smaller frames to reduce thickness and weight.', subtitle: 'Match with Prescription' },
  { img: '/cs_video/scene_clinic.png', duration: 10000, title: 'Material & Comfort', text: 'Frames are available in metal and plastic materials. Lightweight and durable frames improve comfort.', subtitle: 'Material Matters' },
  { img: '/cs_video/scene_procedure.png', duration: 10000, title: 'Final Adjustment', text: 'The frame is adjusted for proper alignment, ensuring it sits straight and comfortable on the face.', subtitle: 'Final Fit' },
  { img: '/cs_video/scene_clinic.png', duration: 5000, title: 'Conclusion', text: 'A well-selected frame enhances vision, comfort, and appearance.', subtitle: 'Perfect Frame Choice' }
];

const A = { bar: 'bg-orange-500', glow: 'shadow-[0_0_18px_rgba(249,115,22,0.7)]', lbl: 'text-orange-400', bright: 'text-orange-500', btn: 'bg-orange-500 hover:bg-orange-400 shadow-orange-500/25', spin: 'bg-orange-500/20', bdr: 'border-orange-500/30 border-t-orange-400', note: 'bg-orange-500/5 border-orange-500/15', nIcon: 'text-orange-400' };

export default function FrameSelectionVideo({ onClose, onStartTest }) {
    ...
    <div className="fixed inset-0 z-[200] bg-[#020617] flex flex-col overflow-hidden">
      <div className="shrink-0 flex items-center justify-between px-4 py-3 bg-[#0a0f1d]/95 backdrop-blur-sm border-b border-white/5 z-30">
        ...
        <div className="text-center">
          ...
          <h1 className="text-[13px] font-bold text-white uppercase tracking-wide">Frame Selection</h1>
        </div>
      ...
      <div className="flex-1 overflow-y-auto">
              ...
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="absolute inset-0 flex flex-col items-center justify-center bg-[#020617] px-6 text-center z-20">
                <div className={`w-14 h-14 ${A.spin} rounded-full flex items-center justify-center border ${A.bdr} mb-4 animate-spin`} style={{ animationDuration: '3s' }}>
                  <div className={`w-10 h-10 ${A.spin} rounded-full flex items-center justify-center animate-spin`} style={{ animationDuration: '2s', animationDirection: 'reverse' }}>
                    <Glasses className={`${A.lbl} animate-pulse`} size={20} />
                  </div>
                </div>
                <p className={`text-[9px] font-black uppercase tracking-[0.3em] mb-1.5 ${A.bright}`}>Dispensing Phase</p>
                <h2 className="text-lg sm:text-2xl text-white font-black tracking-tight uppercase italic mb-2">Frame Selection</h2>
                <p className="text-slate-400 text-xs max-w-xs mb-5 leading-relaxed">Clinical module demonstrating facial analysis and frame mechanics for optimal comfort.</p>
          ...
          {isPlaying && (
            <motion.div key={`t-${currentScene}`} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} transition={{ duration: 0.4, delay: 0.15 }} className="bg-[#020617] px-4 pt-3 pb-4 sm:px-6 sm:pt-4 sm:pb-5">
              <div className="flex items-center justify-between gap-2 mb-2">
                ...
                <span className="text-[9px] font-bold text-emerald-400 bg-emerald-400/10 px-2 py-0.5 rounded-md border border-emerald-400/20 whitespace-nowrap">{data.subtitle || "Aesthetic & Structural Check"}</span>
              </div>
        ...
        {isFinished && (
          <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }} className="px-4 pt-4 pb-6 sm:px-6">
            ...
            <p className="text-slate-400 text-sm leading-relaxed mb-4">Validate your knowledge on frame selection with <strong className="text-white">10 clinical MCQs</strong>.</p>
            ...
            <div className={`mt-4 p-3.5 ${A.note} rounded-xl border`}>
              ...
              <p className="text-slate-400 text-xs leading-relaxed">High minus lenses require smaller eye sizes to minimize edge thickness. A well-fitted rimless frame works beautifully with high index materials, but avoid standard plastic.</p>
            </div>
          </motion.div>
        )}
