    ...
    <div className="fixed inset-0 z-[200] flex flex-col md:items-center md:justify-center bg-[#020617] md:bg-black/95 md:backdrop-blur-xl overflow-y-auto">
      ...
      <div className="relative w-full aspect-[4/5] sm:aspect-video md:max-w-6xl md:rounded-3xl overflow-hidden md:ring-1 md:ring-white/10 md:shadow-2xl shadow-indigo-500/10 shrink-0">
        ...
        {!isPlaying && currentScene === 0 && (
          <div className="absolute inset-0 z-40 flex flex-col items-center justify-center bg-black/60 bg-[url('/amsler_video/amsler_grid.svg')] bg-cover bg-center">
            ...
            <div className="relative z-10 flex flex-col items-center text-center p-8">
              <div className="w-20 h-20 bg-blue-500/20 rounded-full flex items-center justify-center border border-blue-500/30 mb-8 border-t-blue-400 animate-spin" style={{ animationDuration: '3s' }}>
                <div className="w-16 h-16 bg-blue-500/20 rounded-full flex items-center justify-center animate-spin" style={{ animationDuration: '2s', animationDirection: 'reverse' }}>
                  <Eye className="text-blue-400 animate-pulse" size={32} />
                </div>
              </div>
              <span className="text-[10px] font-black text-blue-500 uppercase tracking-[0.4em] mb-4">Central Vision Phase</span>
              <h2 className="text-2xl md:text-5xl text-white font-black mb-4 tracking-tighter uppercase italic">Amsler Grid Test</h2>
              <p className="text-slate-400 font-medium max-w-lg mb-10 leading-relaxed text-sm md:text-base">An immersive clinical module demonstrating the standard assessment of the central visual field and accurate detection of macular abnormalities.</p>
              <button 
                onClick={handleStart}
          ...
          {(isPlaying || (currentScene > 0 && currentScene < SCRIPT.length)) && (
            ...
            >
              <motion.img 
                ...
                animate={currentData.img.includes('svg') ? { scale: [1, 1.02] } : { scale: [1, 1.1], x: [0, -20], y: [0, -10] }}
            ...
            >
              <div className="bg-black/40 backdrop-blur-2xl p-6 md:p-10 rounded-[2rem] border border-white/5 shadow-2xl">
                <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-2 md:gap-4 mb-4">
                  <h3 className="text-blue-400 font-black uppercase tracking-[0.3em] text-[10px] md:text-xs italic">{currentData.title}</h3>
            ...
            <div className="hidden lg:flex flex-col gap-6 w-80 shrink-0">
              <div className="glass-card p-6 bg-blue-500/5 border-blue-500/20 rounded-3xl">
                <h4 className="text-white font-bold mb-2 italic flex items-center gap-2">
                  <Award size={16} className="text-blue-400" /> Macular Note
                </h4>
                <p className="text-xs text-slate-400 leading-relaxed font-medium">Amsler Grid is the gold standard for monitoring metamorphopsia. Instruct patients to report any 'waviness' immediately as it indicates subretinal fluid.</p>
              </div>
            </div>
          </div>
        </div>
