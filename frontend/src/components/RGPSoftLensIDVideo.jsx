...
const A = { bar: 'bg-fuchsia-500', glow: 'shadow-[0_0_18px_rgba(217,70,239,0.7)]', lbl: 'text-fuchsia-400', bright: 'text-fuchsia-500', btn: 'bg-fuchsia-500 hover:bg-fuchsia-400 shadow-fuchsia-500/25', spin: 'bg-fuchsia-500/20', bdr: 'border-fuchsia-500/30 border-t-fuchsia-400', note: 'bg-fuchsia-500/5 border-fuchsia-500/15', nIcon: 'text-fuchsia-400' };
    ...
    <div className="fixed inset-0 z-[200] bg-[#020617] flex flex-col overflow-hidden">
      <div className="shrink-0 flex items-center justify-between px-4 py-3 bg-[#0a0f1d]/95 backdrop-blur-sm border-b border-white/5 z-30">
        ...
        <div className="text-center">
          ...
          <h1 className="text-[13px] font-bold text-white uppercase tracking-wide">RGP vs Soft CL</h1>
        </div>
      ...
      <div className="flex-1 overflow-y-auto">
          ...
          <AnimatePresence>
            {isIdle && (
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="absolute inset-0 flex flex-col items-center justify-center bg-[#020617] px-6 text-center z-20">
                ...
                <h2 className="text-lg sm:text-2xl text-white font-black tracking-tight uppercase italic mb-2">RGP vs Soft CL</h2>
                <p className="text-slate-400 text-xs max-w-xs mb-5 leading-relaxed">Clinical module demonstrating critical structural and fitting differences between rigid and soft lenses.</p>
          ...
          {isPlaying && (
            <motion.div key={`t-${currentScene}`} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} transition={{ duration: 0.4, delay: 0.15 }} className="bg-[#020617] px-4 pt-3 pb-4 sm:px-6 sm:pt-4 sm:pb-5">
              <div className="flex items-center justify-between gap-2 mb-2">
                ...
                <span className="text-[9px] font-bold text-emerald-400 bg-emerald-400/10 px-2 py-0.5 rounded-md border border-emerald-400/20 whitespace-nowrap">{data.subtitle || "Identification Check"}</span>
              </div>
        ...
        {isFinished && (
          <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }} className="px-4 pt-4 pb-6 sm:px-6">
            ...
            <p className="text-slate-400 text-sm leading-relaxed mb-4">Validate your knowledge on RGP vs Soft lenses with <strong className="text-white">10 clinical MCQs</strong>.</p>
            ...
            <div className={`mt-4 p-3.5 ${A.note} rounded-xl border`}>
              <h4 className="text-white font-bold text-xs mb-1 flex items-center gap-1.5"><Award size={12} className={A.nIcon} /> Diagnostic Tip</h4>
              <p className="text-slate-400 text-xs leading-relaxed">RGPs sit largely within the limbal boundary while soft lenses drape over it onto the sclera. RGPs vault astigmatism with tears, correcting it naturally.</p>
            </div>
          </motion.div>
        )}
