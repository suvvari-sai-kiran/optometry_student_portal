...
const SCRIPT = [
  { img: '/cs_video/cs_scene_foggy_road.png', duration: 5000, title: 'Hook', text: 'Can you see clearly… even when the contrast is low?', subtitle: 'Not all vision problems are about clarity…' },
  { img: '/cs_video/scene_clinic.png', duration: 5000, title: 'Introduction', text: 'Welcome to the Contrast Sensitivity Test—an essential part of modern eye care.', subtitle: 'Contrast Sensitivity Test' },
  { img: '/cs_video/scene_chart.png', duration: 8000, title: 'Concept', text: 'Contrast sensitivity measures how well you can distinguish objects from their background, especially when the difference between light and dark is subtle.', subtitle: 'Detecting Light vs Dark' },
  { img: '/cs_video/cs_scene_foggy_road.png', duration: 7000, title: 'Real-Life Importance', text: 'This ability is crucial for everyday tasks like night driving, walking in dim light, or recognizing faces in shadows.', subtitle: 'Night Vision • Low Light • Safety' },
  { img: '/cs_video/scene_chart.png', duration: 10000, title: 'Test Method', text: 'Unlike standard eye charts, this test uses letters or patterns that gradually fade in contrast, not size.', subtitle: 'Letters become lighter, not smaller' },
  { img: '/cs_video/scene_procedure.png', duration: 10000, title: 'Procedure', text: 'The patient reads the chart one eye at a time, identifying the faintest visible letters. The lowest visible contrast determines the score.', subtitle: 'One Eye at a Time' },
  { img: '/cs_video/scene_procedure.png', duration: 10000, title: 'Clinical Insight', text: 'Reduced contrast sensitivity may indicate conditions such as cataracts, glaucoma, or retinal disorders—even when visual acuity appears normal.', subtitle: 'Early Detection Matters' },
  { img: '/cs_video/cs_scene_foggy_road.png', duration: 10000, title: 'Conclusion', text: 'Contrast sensitivity testing reveals how well you truly see in the real world—beyond just reading letters on a chart.', subtitle: 'See Beyond Clarity' },
];

const A = { bar: 'bg-violet-500', glow: 'shadow-[0_0_18px_rgba(139,92,246,0.7)]', lbl: 'text-violet-400', bright: 'text-violet-500', btn: 'bg-violet-500 hover:bg-violet-400 shadow-violet-500/25', spin: 'bg-violet-500/20', bdr: 'border-violet-500/30 border-t-violet-400', note: 'bg-violet-500/5 border-violet-500/15', nIcon: 'text-violet-400' };
    ...
    <div className="fixed inset-0 z-[200] bg-[#020617] flex flex-col overflow-hidden">
      <div className="shrink-0 flex items-center justify-between px-4 py-3 bg-[#0a0f1d]/95 backdrop-blur-sm border-b border-white/5 z-30">
        ...
        <div className="text-center">
          ...
          <h1 className="text-[13px] font-bold text-white uppercase tracking-wide">Contrast Sensitivity</h1>
        </div>
      ...
      <div className="flex-1 overflow-y-auto">
          ...
          <AnimatePresence>
            {isIdle && (
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="absolute inset-0 flex flex-col items-center justify-center bg-[#020617] px-6 text-center z-20">
                ...
                <h2 className="text-lg sm:text-2xl text-white font-black tracking-tight uppercase italic mb-2">Contrast Sensitivity</h2>
                <p className="text-slate-400 text-xs max-w-xs mb-5 leading-relaxed">Clinical module demonstrating the ability to distinguish objects in low-contrast conditions.</p>
        ...
        {isFinished && (
          <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }} className="px-4 pt-4 pb-6 sm:px-6">
            ...
            <p className="text-slate-400 text-sm leading-relaxed mb-4">Validate your knowledge on contrast sensitivity with <strong className="text-white">10 clinical MCQs</strong>.</p>
            ...
            <div className={`mt-4 p-3.5 ${A.note} rounded-xl border`}>
              <h4 className="text-white font-bold text-xs mb-1 flex items-center gap-1.5"><Award size={12} className={A.nIcon} /> Clinical Note</h4>
              <p className="text-slate-400 text-xs leading-relaxed">Contrast sensitivity is early evidence of pathological changes before acuity loss occurs. Follow the patient's reported faintest letters accurately.</p>
            </div>
          </motion.div>
        )}
