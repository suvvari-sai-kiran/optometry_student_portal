...
const SCRIPT = [
  { img: '/cs_video/scene_clinic.png', duration: 3000, title: 'Subjective Refraction', text: 'Subjective Refraction Test.', subtitle: 'Subjective Test' },
  { img: '/cs_video/scene_chart.png', duration: 4000, title: '', text: 'To refine vision based on patient response.', subtitle: 'Patient Feedback' },
  { img: '/cs_video/scene_procedure.png', duration: 5000, title: '', text: 'Patient looks at a visual acuity chart.', subtitle: 'Fixation' },
  { img: '/cs_video/scene_clinic.png', duration: 6000, title: '', text: 'Trial frame with lenses is placed on the patient.', subtitle: 'Trial Frame' },
  { img: '/cs_video/scene_procedure.png', duration: 8000, title: '', text: 'Different lenses are shown one by one. Which is clearer — one or two?', subtitle: 'Lens Comparison' },
  { img: '/cs_video/scene_chart.png', duration: 8000, title: '', text: 'Adjust spherical power for best clarity.', subtitle: 'Sphere Refinement' },
  { img: '/cs_video/scene_procedure.png', duration: 8000, title: '', text: 'Cylinder power and axis are refined using Jackson Cross Cylinder.', subtitle: 'Astigmatism' },
  { img: '/cs_video/scene_chart.png', duration: 8000, title: '', text: 'Final prescription gives best visual acuity.', subtitle: 'Final Vision' },
  { img: '/cs_video/scene_clinic.png', duration: 8000, title: '', text: 'Vision is verified for comfort and clarity.', subtitle: 'Verification' },
  { img: '/cs_video/scene_procedure.png', duration: 7000, title: '', text: 'Subjective refraction ensures accurate vision correction.', subtitle: 'Final Prescription' }
];
...
export default function SubjectiveRefractionVideo({ onClose, onStartTest }) {
    ...
    <div className="fixed inset-0 z-[200] bg-[#020617] flex flex-col overflow-hidden">
      <div className="shrink-0 flex items-center justify-between px-4 py-3 bg-[#0a0f1d]/95 backdrop-blur-sm border-b border-white/5 z-30">
        ...
        <div className="text-center">
          <p className={`text-[9px] font-black uppercase tracking-[0.25em] ${A.bright}`}>Refinement Phase</p>
          <h1 className="text-[13px] font-bold text-white uppercase tracking-wide">Subjective Refraction</h1>
        </div>
      ...
      <div className="flex-1 overflow-y-auto">
          ...
          <AnimatePresence>
            {isIdle && (
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="absolute inset-0 flex flex-col items-center justify-center bg-[#020617] px-6 text-center z-20">
                ...
                <p className={`text-[9px] font-black uppercase tracking-[0.3em] mb-1.5 ${A.bright}`}>Subjective Refinement Phase</p>
                <h2 className="text-lg sm:text-2xl text-white font-black tracking-tight uppercase italic mb-2">Subjective Refraction</h2>
                <p className="text-slate-400 text-xs max-w-xs mb-5 leading-relaxed">Clinical module demonstrating vision refinement through lens comparison and patient response.</p>
        ...
        {isFinished && (
          <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }} className="px-4 pt-4 pb-6 sm:px-6 text-left">
            ...
            <p className="text-slate-400 text-sm leading-relaxed mb-4">Validate your knowledge on subjective vision refinement and lens verification with <strong className="text-white">10 clinical MCQs</strong>.</p>
            ...
            <div className={`mt-4 p-3.5 ${A.note} rounded-xl border`}>
              ...
              <ul className="text-slate-400 text-xs leading-relaxed list-disc list-inside mt-2 space-y-1">
                 <li>Avoid over-minusing the patient; follow the "maximum plus for maximum acuity" rule.</li>
                 <li>Ensure the room is correctly dimmed for the chart illumination.</li>
                 <li>Clearly instruct the patient on the difference between clarity and brightness.</li>
              </ul>
            </div>
          </motion.div>
        )}
