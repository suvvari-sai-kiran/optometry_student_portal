...
const A = { bar: 'bg-teal-500', glow: 'shadow-[0_0_18px_rgba(20,184,166,0.7)]', lbl: 'text-teal-400', bright: 'text-teal-500', btn: 'bg-teal-500 hover:bg-teal-400 shadow-teal-500/25', spin: 'bg-teal-500/20', bdr: 'border-teal-500/30 border-t-teal-400', note: 'bg-teal-500/5 border-teal-500/15', nIcon: 'text-teal-400' };
    ...
    <div className="fixed inset-0 z-[200] bg-[#020617] flex flex-col overflow-hidden">
      <div className="shrink-0 flex items-center justify-between px-4 py-3 bg-[#0a0f1d]/95 backdrop-blur-sm border-b border-white/5 z-30">
        ...
        <div className="text-center">
          ...
          <h1 className="text-[13px] font-bold text-white uppercase tracking-wide">Lens Fitting</h1>
        </div>
      ...
      <div className="flex-1 overflow-y-auto">
          ...
          <AnimatePresence>
            {isIdle && (
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="absolute inset-0 flex flex-col items-center justify-center bg-[#020617] px-6 text-center z-20">
                ...
                <h2 className="text-lg sm:text-2xl text-white font-black tracking-tight uppercase italic mb-2">Lens Fitting</h2>
                <p className="text-slate-400 text-xs max-w-xs mb-5 leading-relaxed">Clinical module demonstrating proper lens fit evaluation and interpretation.</p>
        ...
        {isFinished && (
          <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }} className="px-4 pt-4 pb-6 sm:px-6">
            ...
            <p className="text-slate-400 text-sm leading-relaxed mb-4">Validate your knowledge on lens fitting with <strong className="text-white">10 clinical MCQs</strong>.</p>
            ...
            <div className={`mt-4 p-3.5 ${A.note} rounded-xl border`}>
              ...
              <p className="text-slate-400 text-xs leading-relaxed">The push-up test gives a direct read on lens tightness. A truly tight lens can compromise oxygen flow and cause long term corneal complications!</p>
            </div>
          </motion.div>
        )}
